...
cc.Class({
    ...
    InitPayLine() {
        ...
        this.payLine[6] = this.GetPosByColum([0, 1, 7, 3, 4 ]);
        this.payLine[7] = this.GetPosByColum([10, 11, 7, 13, 14]);
        this.payLine[8] = this.GetPosByColum([5, 11, 12, 13, 9]);
        this.payLine[9] = this.GetPosByColum([5, 1, 2, 3, 9]);
        this.payLine[10] =this.GetPosByColum( [ 5, 1, 7, 3, 9 ]);
        this.payLine[11] = this.GetPosByColum([ 5, 11, 7, 13, 9 ]);
        this.payLine[12] = this.GetPosByColum([ 0, 6, 2, 8, 4 ]);
        this.payLine[13] = this.GetPosByColum([10, 6, 12, 8, 14 ]);
        this.payLine[14] = this.GetPosByColum([ 5, 6, 2, 8, 9 ]);
        this.payLine[15] = this.GetPosByColum([ 5, 6, 12, 8, 9]);
        this.payLine[16] = this.GetPosByColum([ 0, 6, 7, 8, 4]);
        this.payLine[17] = this.GetPosByColum([ 10, 6, 7, 8, 14]);
        this.payLine[18] = this.GetPosByColum([ 10, 1, 2, 3, 14 ]);
        this.payLine[19] = this.GetPosByColum([ 0, 11, 12, 13, 4 ]);
        this.payLine[20] = this.GetPosByColum([ 0, 11, 2, 13, 4]);
    },
    ...
    ShowLineWin(listLine) {
        ...
        for(let i = 0; i < listLine.length; i++) {
            ...
            this.toDoList.AddWork(()=>{
                let animationName ="line"+listLine[i];
                this.listLineImg[listLine[i]-1].active = true;
                this.listLineImg[listLine[i]-1].getComponent(sp.Skeleton).setAnimation(0, animationName, false);;
            }, false);
    ...
    ShowAllLineStart(){
        ...
        for(let i = 0; i < this.listLineImg.length; i++) {
            this.toDoList.AddWork(()=>{
                let animationName ="line"+(i+1);
                this.listLineImg[i].active = true;
                this.listLineImg[i].getComponent(sp.Skeleton).setAnimation(0, animationName, false);  
            }, false);
        }
