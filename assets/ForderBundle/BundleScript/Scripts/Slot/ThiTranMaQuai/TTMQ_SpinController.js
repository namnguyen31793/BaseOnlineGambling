var TT_ResponseCode = require('TTMQ_ResponseCode');
var TTMQ_SpinController = cc.Class({
    statics: {
        getIns() {
            if (this.self == null)
            {
                this.self = new TTMQ_SpinController();
                this.self.Setup();
            }
            return this.self;
        }
    ...
    Setup()
    {
        ...
        this.lineIndex = 25;
        this.lineInfo = "";
        this.X2GameController = require("X2GameController").getIns();
        this.roomID = 1;
    },
        ...
        ResponseNetwork(code, packet) {
            switch (code) {
                case TT_ResponseCode.MSG_SERVER_GET_ROOM_CONFIG:
                    this.ServerResponse_Handle_GetRoomConfig(packet);
                    break;
                case TT_ResponseCode.MSG_SERVER_THAN_TAI_GAME_GET_ACCOUNT_INFO:
                   this.ServerResponse_Handle_GetAccountInfo(packet);
                    break;
                case TT_ResponseCode.MSG_SERVER_THAN_TAI_GAME_JACKPOT_INFO:
                    this.ServerResponse_Handle_GetJackpotInfo(packet);
                    break;
                case TT_ResponseCode.MSG_SERVER_THAN_TAI_GAME_SPIN:
                    cc.log(packet);
                    this.ServerResponse_Handle_GetSpinResult(packet);
                    break;
                case TT_ResponseCode.MSG_SERVER_THAN_TAI_GAME_GET_DETAIL_HISTORY:
                    this.GetHistory(packet);
                    break;
                case TT_ResponseCode.MSG_SERVER_THAN_TAI_GAME_GET_TOP_TAKE_JACKPOT_INFO:
                    this.GetTop(packet);
                    break;
                case TT_ResponseCode.MSG_SERVER_THAN_TAI_GAME_X2_REWARD:
                    this.X2GameController.Handle_X2Game_Response(packet);
                    break;
                case TT_ResponseCode.MSG_SERVER_THAN_TAI_GAME_SPIN_CHOI_THU:
        ...
        {
           cc.log(">>>> ServerResponse_Handle_GetRoomConfig");
        ...
        ServerResponse_Handle_GetSpinResult(packet)
        {
            ...
            require("BalanceController").getIns().subtractBalanceUI(totalBet);
        ...
        ServerResponse_Handle_ChoiThu(packet)
        {
            ...
            this.spinView.updateJackpotUI(currentJackpotValue);
            require("BalanceController").getIns().subTryBalance(totalBet);
        ...
        CheckNotEnoughMoney()
        {
            ...
            if(Global.MainPlayerInfo.ingameBalance >= (this.CurrentRoomConfig.Bet*currentLine))
                return true;
            return false;
        },
...
module.exports = TTMQ_SpinController;