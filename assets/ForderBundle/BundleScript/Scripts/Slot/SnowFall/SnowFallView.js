...
cc.Class({
    ...
    Init() {
        this.slotType = Global.Enum.GAME_TYPE.SNOW_FALL;
        this.lineData = 50;
    },
    ...
    DropMatrix(matrix, winMoneyStep, index){
        this.spinManager.DropMatrix(matrix, winMoneyStep, index);
    },
    ...
    SetLastPrizeDrop(num){
        this.menuView.UpdateWinValue(num*this.GetBetValue());
    },
    ...
    UpdateMoneyResult(winNormalValue, extend, totalValue, isTakeJackpot, isWaitRunMoneyWin = false) {
            ...
            if(winNormalValue > 0) {
                if(!isBigWin) {
                    ...
                    if(isWaitRunMoneyWin) {
                        this.scheduleOnce(()=>{
                            this.menuView.UpdateWinValue(winNormalValue-winNormalValue/extend);
                            this.toDoList.DoWork();
                        } , 1);  
                    } else {
                        this.menuView.UpdateWinValue(winNormalValue-winNormalValue/extend);
    ...
    ShowMultiEffect(extend){
        this.normalManager.ShowMultiEffect(extend);
    },
