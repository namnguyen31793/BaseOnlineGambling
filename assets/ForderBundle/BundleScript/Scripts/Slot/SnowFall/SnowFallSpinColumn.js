...
cc.Class({
    ...
    FallOut(item,delayTime, timeFall) {
        ...
        item.runAction(cc.sequence(cc.delayTime(delayTime),cc.moveTo(timeFall, cc.v2(item.x, item.y - this.distanceY * 5)))).setTag(1);
    ...
    FallItem(item,delayTime, timeFall, indexRow, step) {
        ...
        item.stopActionByTag(1);
        item.stopActionByTag(2);
        item.runAction(cc.sequence(cc.delayTime(delayTime),cc.moveTo(timeFall, cc.v2(item.x, y - (TOTAL_DIS_UP-(4-indexRow)*SUB_DIS_UP) - this.distanceY * step)))).setTag(1);
    ...
    GetResult(index, matrix) {
        ...
        let length = this.listItem.length;
        for(let i = 0; i < length; i++) {
            this.listItem[i].setPosition(cc.v2(this.listItem[i].x, this.startPos-i*this.distanceY+this.distanceY*4));
        }
        for(let i = 0; i < length; i++) {
            this.FallItem(this.listItem[length-i-1], i*this.TIME_DELAY_ITEM, this.TIME_FALL_ITEM, i, 4);
        }
        this.scheduleOnce(()=>{
    ...
    AddBelowItem(step, currentIndex, matrix, indexDelay) {
        ...
        this.FallItem(this.listItem[this.numbAbove-1], indexDelay * this.TIME_DELAY_2, this.TIME_FALL_2 * step/4 / ((4-indexRow)/4), indexRow, step);
